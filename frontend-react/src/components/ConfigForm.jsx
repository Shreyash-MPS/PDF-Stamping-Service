...
const TABS = [
    ...
    { key: 'leftMargin', label: 'Left Margin', icon: PanelLeft },
    { key: 'rightMargin', label: 'Right Margin', icon: PanelRight },
];
...
/* Build a summary of what's enabled in a section */
...
const PreviewModal = ({ sections, templateName, onTemplateChange, onClose }) => {
    ...
    /* Build a composite preview showing all enabled sections on an A4-sized "PDF page" */
    const buildPreviewHtml = () => {
        ...
        if (newPageSection.enabled && template) {
            newPageHtml = template.htmlTemplate.replace('{{DATE}}', date);
        ...
        const leftHtml = leftSection.enabled
            ? `<div class="stamp-margin stamp-left"><span>Left Margin</span></div>` : '';

        const rightHtml = rightSection.enabled
            ? `<div class="stamp-margin stamp-right"><span>Right Margin</span></div>` : '';

        const position = newPageSection.pagePosition ?? 'front';

        const pdfPlaceholder = `<div class="pdf-content">
            <div style="text-align:center;">
                <div style="font-size:40px;margin-bottom:8px;">📄</div>
                <div>Original PDF Content</div>
                <div style="font-size:12px;margin-top:4px;">Pages would appear here</div>
            </div>
        </div>`;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
            <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-200">
                    ...
                    <button onClick={onClose} className="p-1.5 text-gray-400 hover:text-gray-700 rounded-md hover:bg-gray-100 cursor-pointer">
                        <X className="w-5 h-5" />
                    </button>
                </div>
                ...
                <div className="px-5 py-3 border-b border-gray-100 bg-[#fafafa]">
                    ...
                    <select
                        className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#a81732]/20 focus:border-[#a81732]"
                        value={templateName}
                ...
                <div className="px-5 py-2.5 border-b border-gray-100 flex flex-wrap gap-2">
                    {TABS.map(t => {
                        ...
                        return (
                            <span key={t.key} className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium bg-[#fdf2f4] text-[#a81732] border border-[#a81732]/20">
                                <span className="w-1.5 h-1.5 rounded-full bg-[#a81732]" />
...
const ConfigForm = () => {
    ...
    const handleSave = async () => {
        ...
        if (isNewJournal && jName) {
            addJournalToPublisher(pubId, { jId, jName });
        }
        const configData = { pubId, jId, templateName, ...sections };
        if (isEdit) {
            updateConfig(Number(id), configData);
        } else {
            addConfig(configData);
        }

        // Save config + template to backend configs/ folder
        const result = await saveConfigToBackend(pubId, jId, sections, templateName);
        if (result.success) {
            showToast(isEdit ? 'Configuration updated and saved to server.' : 'Configuration created and saved to server.');
        } else {
            showToast(isEdit ? 'Configuration updated locally. Server save failed.' : 'Configuration created locally. Server save failed.', true);
        }
    ...
    return (
        <div className="min-h-screen bg-[#f5f5f5]">
            <div className="max-w-4xl mx-auto px-6 py-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-3">
                        <button onClick={() => setShowPreview(true)}
                            className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-semibold text-[#a81732] border border-[#a81732] rounded-md hover:bg-[#fdf2f4] transition-colors cursor-pointer">
                            <Eye className="w-4 h-4" />
                            View Preview
                        </button>
                        <button onClick={handleSave}
                            className="inline-flex items-center gap-2 px-5 py-2.5 text-sm font-semibold text-white bg-[#a81732] rounded-md hover:bg-[#851227] transition-colors cursor-pointer">
                ...
                {/* Toast */}
                {toast && (
                    <div className={`mb-4 px-4 py-2.5 rounded-md text-sm font-medium ${
                        toast.isError ? 'bg-red-50 text-red-700 border border-red-200' : 'bg-green-50 text-green-700 border border-green-200'
                    }`}>{toast.msg}</div>
                )}

                {/* Publisher / Journal row */}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            {!isNewPublisher ? (
                                <div className="flex gap-2">
                                    <SearchableSelect
                                        ...
                                        value={pubId}
                                        onChange={v => { setPubId(v); setJId(''); setIsNewJournal(false); }}
                                        placeholder="Select publisher..."
                                        disabled={isEdit}
                                    />
                                    {!isEdit && (
                                        <button onClick={() => { setIsNewPublisher(true); setPubId(''); setJId(''); }}
                        ...
                        <div>
                            ...
                            {!isNewJournal ? (
                                <div className="flex gap-2">
                                    <SearchableSelect
                                        options={journals.map(j => ({ value: j.jId, label: j.jName }))}
                                        value={jId}
                                        onChange={v => setJId(v)}
                                    ...
                                    />
                                    {!isEdit && pubId && (
                                        <button onClick={() => { setIsNewJournal(true); setJId(''); }}
                                            className="px-2 py-1.5 text-xs text-[#a81732] border border-[#a81732] rounded-md hover:bg-[#fdf2f4] transition-colors cursor-pointer whitespace-nowrap">+ New</button>
                                    )}
                                </div>
                            ) : (
                                <div className="space-y-1.5">
                                    <input type="text" placeholder="Journal ID" className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#a81732]/20 focus:border-[#a81732]"
                                    ...
                                    <button onClick={() => { setIsNewJournal(false); setJId(''); }} className="text-xs text-gray-500 hover:text-gray-700 cursor-pointer">← Existing</button>
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            return (
                                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                                    className={`flex items-center gap-2 px-5 py-3 text-sm font-semibold whitespace-nowrap border-b-2 transition-colors cursor-pointer ${
                                        isActive ? 'border-[#a81732] text-[#a81732] bg-white' : 'border-transparent text-gray-500 hover:text-gray-700 hover:bg-gray-50'
                                    }`}>
                ...
                <div className="flex gap-3 mt-5">
                    <button onClick={() => navigate('/')}
                        className="flex-1 px-4 py-2.5 text-sm font-semibold text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors cursor-pointer">
                        Cancel
                    </button>
                    <button onClick={() => setShowPreview(true)}
                        className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-[#a81732] bg-white border border-[#a81732] rounded-md hover:bg-[#fdf2f4] transition-colors cursor-pointer">
                        <Eye className="w-4 h-4" />
                        View Preview
                    </button>
                    <button onClick={handleSave}
                        className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold text-white bg-[#a81732] rounded-md hover:bg-[#851227] transition-colors cursor-pointer">
                        <Save className="w-4 h-4" />
